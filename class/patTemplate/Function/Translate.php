	...
	function call( $params, $content )
	{
		...
		if( empty( $content ) )
		{
			return;
                ...
                if(1){
                    //SPEED: cortocircuitamos este sistema, porque en esta instalacion no se usa
                    return $content;
                }
	...
	function _loadTranslationFile( $input )
	{
		...
		foreach( $this->_globalconfig['lang'] as $lang )
		{
			...
			if( is_array( $tmp ) )
			{
				...
				$olddata= $_SESSION["traducciones_" . $lang];
				
				if ( is_array($olddata) ){
					$tmp = array_merge($olddata,$tmp);
				}				
				
				$_SESSION["traducciones_" . $lang] = $tmp;//Tei, put translation data in session data.
