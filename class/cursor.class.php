...
class Cursor {
	...
	function Cursor() {
        ...
        function Config($idFila,$nameid,$nombretabla){
		$this->_nameid			= $nameid;
	...
	function LoadTable($tabla, $nameid, $id) {
		...
		$myresult = query($sql);//,"From:" . $_SERVER["SCRIPT_FILENAME"] );
	...
	// sets
	function set($key, $valor, $force = true) {
                if (is_array($key)) {
                    // //TODO: esto esta bien? deberia ser $key
                    $valor = $valor[$key];
                    
                    die("TODO: Se usa para key:$key,valor:$valor--");
                }
	...
	function setId($id){
                ...
                if (!$this->_nameid)
                    throw new Exception("Clase no tiene definida la clave principal");

		$this->_idFila = CleanID($id);
                $this->set($this->_nameid,$this->_idFila);
	}
	...
	function Save($hackCheckBox = false,$desdeDonde=false) {
		//Si no hay nada que salvar, hecho! :I
                $numCambios = 0;
